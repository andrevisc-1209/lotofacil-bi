"""
duplasena_atualizar.py
------------------------
Coleta e atualiza os sorteios da Dupla Sena no Supabase (fonte primária) e no
SQLite local (fallback/backup) — mesmo padrão de megasena_atualizar.py e
lotomania_atualizar.py, usando as MESMAS credenciais do Supabase da Lotofácil
(só a tabela muda, ver duplasena_db.TABELA).

Uso:
    python duplasena_atualizar.py                  # atualiza Supabase + SQLite local (só os novos)
    python duplasena_atualizar.py --init-all        # carga total: concurso 1 até o último disponível
    python duplasena_atualizar.py --only-local      # atualiza só o SQLite (offline)
    python duplasena_atualizar.py --source local    # descobre o delta pelo SQLite, não pelo Supabase
...
(reaproveitado por duplasena_db, que só troca a tabela alvo).
"""
...
import duplasena_db as db_module
...
def baixar_lote(numeros: list[int]) -> list[dict]:
    ...
    em megasena_atualizar.py e lotomania_atualizar.py."""
    if not numeros:
        return []
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Atualiza os sorteios da Dupla Sena no Supabase e/ou SQLite local"
    )
    parser.add_argument("--db", default="duplasena.db", help="Caminho do banco SQLite local (padrão: duplasena.db)")
